...
/// <summary>
/// Dedicated lease/heartbeat trace. Lease expiry is TIME-DRIVEN, so this uses the
/// driven clock (Tick → debug.tick) with far-future promise timeouts (so the
/// wall-clock background promise-timeout loop never interferes). Verified facts:
///   acquire ttl=T at now=N → lease expires at N+T; Tick to >= N+T reclaims the
///   task to pending (version UNCHANGED); heartbeat (matching pid) resets the
///   lease; wrong-pid heartbeat is a no-op.
/// </summary>
public static class LeaseAndTimeoutTests
{
    public static async Task<int> Run(Harness harness)
    {
        ...
        await harness.Client.DebugTick(harness.Now);   // seed the driven clock

        var checker = new TraceRunner(harness.Spec, harness.Client, harness.Now);
        long far = 9_000_000_000_000;      // promise timeouts far beyond any tick

        // --- lease expiry reclaims an acquired task ---
        ...
        // A READ past the deadline must be accepted whichever way the server
        // answers. Reclaim is a scheduling CHOICE, not a fact, so a read may not
        // reveal it: an eager sweeper reports `pending`, a server that reclaims
        // lazily at acquire still reports `acquired` with the old pid and ttl.
        // Both are conformant, and the model carries both worlds until something
        // forces the issue. (Before R5 was armed as an obligation, the model
        // demanded `pending` here and called the lazy server non-conformant.)
        ...
        // --- wrong-pid heartbeat is a no-op (task still expires) ---
        ...
        // --- wrong-VERSION heartbeat is a no-op (T-05: version must match) ---
        // H is held by wX at v2 since 1014000, lease → 1019000. A v1 heartbeat
        // from the CORRECT pid must not extend it.
